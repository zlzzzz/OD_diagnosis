...
def diag_and_disa(pta_table, age, gender, work_age):
    ...
    # 校正矩阵 adjust_male_row， adjust_female_row
...
def rept_test(pta_table):
    pta_table = np.array(pta_table)
    r500 = max(pta_table[0][0], pta_table[2][0], pta_table[4][0])-min(pta_table[0][0], pta_table[2][0], pta_table[4][0])
    r1000 = max(pta_table[0][1], pta_table[2][1], pta_table[4][1])-min(pta_table[0][1], pta_table[2][1], pta_table[4][1])
    r2000 = max(pta_table[0][2], pta_table[2][2], pta_table[4][2])-min(pta_table[0][2], pta_table[2][2], pta_table[4][2])
    r3000 = max(pta_table[0][3], pta_table[2][3], pta_table[4][3])-min(pta_table[0][3], pta_table[2][3], pta_table[4][3])
    r4000 = max(pta_table[0][4], pta_table[2][4], pta_table[4][4])-min(pta_table[0][4], pta_table[2][4], pta_table[4][4])
    r6000 = max(pta_table[0][5], pta_table[2][5], pta_table[4][5])-min(pta_table[0][5], pta_table[2][5], pta_table[4][5])

    l500 = max(pta_table[1][0], pta_table[3][0], pta_table[5][0])-min(pta_table[1][0], pta_table[3][0], pta_table[5][0])
    l1000 = max(pta_table[1][1], pta_table[3][1], pta_table[5][1])-min(pta_table[1][1], pta_table[3][1], pta_table[5][1])
    l2000 = max(pta_table[1][2], pta_table[3][2], pta_table[5][2])-min(pta_table[1][2], pta_table[3][2], pta_table[5][2])
    l3000 = max(pta_table[1][3], pta_table[3][3], pta_table[5][3])-min(pta_table[1][3], pta_table[3][3], pta_table[5][3])
    l4000 = max(pta_table[1][4], pta_table[3][4], pta_table[5][4])-min(pta_table[1][4], pta_table[3][4], pta_table[5][4])
    l6000 = max(pta_table[1][5], pta_table[3][5], pta_table[5][5])-min(pta_table[1][5], pta_table[3][5], pta_table[5][5])

    pta_max = max(r500, r1000, r2000, r3000, r4000, r6000, l500, l1000, l2000, l3000, l4000, l6000)

    if pta_max > 10:
        rt_result = {'rt': False, 'inf': "纯音听阈各频率重复性测试结果阈值偏差过大。"}
    else:
        rt_result = {'rt': True, 'inf': "纯音听阈各频率重复性测试结果阈值偏差符合要求。"}
    ...
    low = np.array([[r500, r1000, r2000, l500, l1000, l2000]])
    low_mean = round(np.mean(low))  # 用的是平均听阈损失
    ...
    hight = np.array([[r3000, r4000, r6000, l3000, l4000, l6000]])
    hight_mean = round(np.mean(hight))  # 用的是平均听阈损失

    if low_mean >= hight_mean:
        rt_result['rt'] = False
        rt_result['inf2'] = "语频听力损失（平均）大于等于高频听力损失（平均），不应诊断职业性噪声聋"
    else:
        rt_result['inf2'] = "语频听力损失（平均）小于高频听力损失（平均）"
    '''
    返回结果是一个dict，keys包括'rt', 'inf', 'inf2'
    可以用['rt']调取结果
    '''
    return rt_result
